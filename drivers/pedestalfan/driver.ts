...
import PedestalFan from "../../tsvesync/devices/towerfan/PedestalFan";

class PedestalFanDriver extends Homey.Driver {
    ...
    async onPair(session: any) {
        ...
        session.setHandler("list_devices", async () => {
            ...
            let devicesList: any = [];
            devices.filter(d => {
                return d instanceof PedestalFan;
            })
                .forEach((d) => {
                    if (d instanceof PedestalFan) {
                        devicesList.push({
...
module.exports = PedestalFanDriver;
